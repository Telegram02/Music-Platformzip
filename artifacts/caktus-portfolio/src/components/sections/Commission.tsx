...
const GENRE_ICONS: Array<{ name: string; label: string; icon: LucideIcon }> = [
  { name: "Guitar",     label: "Rock / Metal",  icon: Guitar },
  { name: "Mic",        label: "Rap / Hip-Hop",  icon: Mic },
  { name: "Cpu",        label: "Electronic",     icon: Cpu },
  { name: "Gamepad2",   label: "Video Games",    icon: Gamepad2 },
  { name: "Skull",      label: "Horror",         icon: Skull },
  { name: "Flame",      label: "Action",         icon: Flame },
  { name: "Compass",    label: "Adventure",      icon: Compass },
  { name: "Film",       label: "Cinematic",      icon: Film },
  { name: "Waves",      label: "Ambient",        icon: Waves },
  { name: "Radio",      label: "Synthwave",      icon: Radio },
  { name: "Drum",       label: "Percussion",     icon: Drum },
  { name: "Zap",        label: "Industrial",     icon: Zap },
  { name: "Headphones", label: "Lo-fi",          icon: Headphones },
  { name: "Volume2",    label: "Sound Design",   icon: Volume2 },
  { name: "Star",       label: "Featured",       icon: Star },
  { name: "Music2",     label: "Other / General",icon: Music2 },
];
...
export function Commission() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const genreEntry = GENRE_ICONS.find((g) => g.name === form.genre);
    ...
    const message = [
      ...
      form.genre ? `GENRE: ${genreEntry?.label ?? form.genre}` : null,
          ...
          {submitted ? (
            ...
            >
                ...
                <div className="flex flex-wrap gap-2">
                  {PROJECT_TYPES.map((pt) => (
                    <button
                      key={pt.id}
                      type="button"
                      onClick={() => toggle("projectType", pt.id)}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {GENRE_ICONS.map(({ name, label, icon: Icon }) => (
                    <button
                      key={name}
                      type="button"
