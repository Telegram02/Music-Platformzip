...
export const api = {
  ...
  updateSettings: (data: Record<string, string>) =>
    req<{ ok: boolean }>("PUT", "/settings", data),

  // Tracks
  getTracks: (all = false) =>
    req<AudioTrack[]>("GET", all ? "/tracks/all" : "/tracks"),
  createTrack: (data: Partial<AudioTrack>) =>
    req<AudioTrack>("POST", "/tracks", data),
  updateTrack: (id: number, data: Partial<AudioTrack>) =>
    req<AudioTrack>("PUT", `/tracks/${id}`, data),
  deleteTrack: (id: number) => req<void>("DELETE", `/tracks/${id}`),

  // Portfolio
  getPortfolio: (all = false) =>
    req<PortfolioItem[]>("GET", all ? "/portfolio/all" : "/portfolio"),
  createPortfolioItem: (data: Partial<PortfolioItem>) =>
    req<PortfolioItem>("POST", "/portfolio", data),
  updatePortfolioItem: (id: number, data: Partial<PortfolioItem>) =>
    req<PortfolioItem>("PUT", `/portfolio/${id}`, data),
  deletePortfolioItem: (id: number) =>
    req<void>("DELETE", `/portfolio/${id}`),

  // Social links
  getSocial: (all = false) =>
    req<SocialLink[]>("GET", all ? "/social/all" : "/social"),
  createSocialLink: (data: Partial<SocialLink>) =>
    req<SocialLink>("POST", "/social", data),
  updateSocialLink: (id: number, data: Partial<SocialLink>) =>
    req<SocialLink>("PUT", `/social/${id}`, data),
  deleteSocialLink: (id: number) => req<void>("DELETE", `/social/${id}`),

  // Upload
  requestUploadUrl: (name: string, size: number, contentType: string) =>
    req<{ uploadURL: string; objectPath: string }>(
      "POST",
