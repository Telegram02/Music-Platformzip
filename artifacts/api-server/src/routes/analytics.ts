...
router.get("/analytics", requireAdmin, async (_req: Request, res: Response) => {
  try {
    ...
    const [[total], [last30], [last7], [today], [unique], topPages, daily] = await Promise.all([
      ...
      db.select({ path: siteVisitsTable.path, count: count() })
        .from(siteVisitsTable)
        .groupBy(siteVisitsTable.path)
        .orderBy(desc(count()))
        .limit(8),
      db.select({ day: sql<string>`DATE(created_at)`, count: count() })
        .from(siteVisitsTable)
        .where(gte(siteVisitsTable.createdAt, fourteenAgo))
    ...
    res.json({
      ...
      last7Days: last7.count,
      today: today.count,
      uniqueVisitors30: unique.count,
      topPages,
      daily,
    });
  } catch (e) {
