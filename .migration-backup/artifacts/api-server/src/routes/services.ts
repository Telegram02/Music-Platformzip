...
router.put("/services/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [row] = await db.update(servicesTable)
    .set({ ...(iconName !== undefined && { iconName }), ...(title !== undefined && { title }),
      ...(description !== undefined && { description }), ...(colorClass !== undefined && { colorClass }),
      ...(sortOrder !== undefined && { sortOrder }), ...(active !== undefined && { active }) })
